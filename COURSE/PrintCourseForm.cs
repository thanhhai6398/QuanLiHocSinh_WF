...
namespace ProjectQLSV
{
    public partial class PrintCourseForm : Form
    {
        ...
        {
            SqlCommand command = new SqlCommand("SELECT Id as ID, label as Label, period as Period, description as Description from Course");
            fillGrid(command);
        }
        public void fillGrid(SqlCommand command)
        {
            dataGridViewPrint.ReadOnly = true;
            dataGridViewPrint.RowTemplate.Height = 60; //co gang de pic dep, dang tim auto-size
            dataGridViewPrint.DataSource = course.getAllCourse(command);
        ...
        public void Data_To_Word(DataGridView DGV, string filename)
        {
            if (DGV.Rows.Count != 0)
            {
                ...
                foreach (Microsoft.Office.Interop.Word.Section section in oDoc.Application.ActiveDocument.Sections)
                {
                    ...
                    headerRange.Text = "TRƯỜNG ĐẠI HỌC SƯ PHẠM KỸ THUẬT TPHCM" + Environment.NewLine + $"Khoa: CÔNG NGHỆ THÔNG TIN" + Environment.NewLine + $"DANH SÁCH CÁC KHÓA HỌC" + Environment.NewLine + $"Lớp: IT03";
